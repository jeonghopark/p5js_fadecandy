...
function drawFrame() {
    ...
    if (showPixelLocations === true) {
        for (i = 0; i < leds; i++) {
            ...
            // rect(ledXpoints[i]+1, ledYpoints[i]+1, 10, 10);
        }
    }
}
...
function pixelPos5x5(pos) {
    let lineNum = 22;
    let columnPos = 0;

    if (pos != 0) {
        packet[(pos + lineNum * int((pos - 1) / 5) - int((pos - 1) / 5) * 5) * 3 + 1] = 255;
        packet[(pos + lineNum * int((pos - 1) / 5) - int((pos - 1) / 5) * 5) * 3 + 2] = 255;
        packet[(pos + lineNum * int((pos - 1) / 5) - int((pos - 1) / 5) * 5) * 3 + 3] = 255;
    }
